...
lasso_cv_adjusted <- function(x, y, cv_iter, alpha, pen=NULL, measure = c("mse", "deviance", "class", "auc", "mae"),
                         family=c("gaussian", "binomial", "poisson", "multinomial", "cox", "mgaussian"),run_parallel=TRUE) {
  ...
  if (run_parallel) {
    stopImplicitCluster()  # Optional: Stop the parallel backend
  }
...
## cross-validation, which is intended to stabilize the method and to find the optimal lambda with the smallest error.
#' @export
lasso_cv <- function(x, y, cv_iter, alpha, pen=NULL, measure = c("mse", "deviance", "class", "auc", "mae"),
                     family=c("gaussian", "binomial", "poisson", "multinomial", "cox", "mgaussian"),run_parallel=TRUE) {
  run_parallel = FALSE
  if (run_parallel) {
  ...
  for (i in 1:cv_iter) {
    if (is.null(pen))
      #perform 1 cross-validation (default: 10 folds)
      cv <- cv.glmnet(x, y, alpha=alpha, family=family, type.measure=measure,
                      dfmax=dfmax, type.multinomial="grouped",parallel=run_parallel)
    #plot(cv, xvar = "dev")
    else
      ##perform 1 cross-validation (default: 10 folds)
      cv <- cv.glmnet(x, y, alpha=alpha, family=family, type.measure=measure,
                      penalty.factor=pen, dfmax=dfmax, type.multinomial="grouped",parallel=run_parallel)
    CVMs  <- qpcR:::cbind.na(CVMs,  cv$cvm) #store partial likelihood deviances
    CVSDs <- qpcR:::cbind.na(CVSDs, cv$cvsd)#store standard errors
    lambda.mins = c(lambda.mins,cv$lambda.min) #store lambda.min for each run
    lambda.1ses = c(lambda.1ses,cv$lambda.1se) #store lambda.1se for each run
